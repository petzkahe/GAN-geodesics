from GAN.swiss_roll.geodesic_graph_nn import *
...
def find_geodesics_nn(method, sess, training,train_writer):
    ...
    if method=="proposed":
        ...
        start_end = np.concat(z_start_center,z_end_center,axis = 0).reshape(1,2*dim_latent)
    ...
    with tf.Session() as session:
        ...
        if do_no_training:
            ...
            for method in methods:
                ...
                if method == "Jacobian":
                    curves_in_latent_space_value, curves_in_sample_space_value, objective_values = find_geodesics_nn(method, session, train_geodesic_Jacobian,None)
                    print('Jacobian done!')
                elif method == "proposed":
                    train_writer  =  tf.summary.FileWriter( './graphs',session.graph)
                    curves_in_latent_space_value, curves_in_sample_space_value, objective_values = find_geodesics_nn(method, session, train_geodesic_proposed,train_writer)
                    print('Proposed done!')
                    
                elif method == "linear":
                    curves_in_latent_space_value, curves_in_sample_space_value, objective_values = find_geodesics_nn(method, session, None,None)
